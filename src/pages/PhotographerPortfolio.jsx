...
const PhotographerPortfolio = () => {
  ...
  const handleSubmit = async () => {
      ...
      try {
          ...
          if (localStorage.getItem("token") === "mock-creator-token") {
            // Just navigate next
            // Maybe save to local storage for demo
            alert("Saved (Mock)!");
            // navigate("/dashboard"); 
            navigate("/portfolio/media"); // Corrected flow
            return;
          }
          ...
          const payload = {
            userId,
            ...form,
             // Location needs handling if we use the select, for now sending empty or keeping existing
             location: { type: 'Point', coordinates: [form.longitude || 0, form.latitude || 0] } 
          };
