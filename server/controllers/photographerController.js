...
exports.getAllPhotographers = async (req, res) => {
    try {
        const photographers = await Photographer.find().populate('userId', 'name email profilePic rating reviewCount');
...
exports.getPhotographerById = async (req, res) => {
    try {
        ...
        let photographer = await Photographer.findOne({ userId: id }).populate('userId', '-password');

        if (!photographer) {
            // Fallback: maybe id IS the photographer ID?
            photographer = await Photographer.findById(id).populate('userId', '-password');
        }
